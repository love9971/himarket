...
FRONTEND_HOST="${FRONTEND_HOST:-localhost:5173}"
ADMIN_HOST="${ADMIN_HOST:-localhost:5173}"
ADMIN_TOKEN=""
DEVELOPER_ID=""
PORTAL_ID=""
CONSUMER_ID=""
...
step_1_admin_login() {
  ...
  while (( attempt <= MAX_RETRIES )); do
    log "执行步骤 1: 管理员登录 (第 ${attempt}/${MAX_RETRIES} 次)..."
...
########################################
# 步骤 2: 确保 Portal 存在（查询或创建）
########################################
step_2_ensure_portal() {
  local extra_args="-H 'Authorization: Bearer ${ADMIN_TOKEN}'"
  
  local attempt=1
  while (( attempt <= MAX_RETRIES )); do
    log "执行步骤 2: 确保 Portal 存在 (第 ${attempt}/${MAX_RETRIES} 次)..."
    
    if call_api "查询Portal列表" "GET" "${ADMIN_HOST}" "/api/v1/portals" "" "$extra_args"; then
      PORTAL_ID=$(extract_json_field "$API_RESPONSE" "portalId")
      
      if [[ -n "$PORTAL_ID" ]]; then
        log "Portal 已存在，Portal ID: ${PORTAL_ID}"
        return 0
      fi
    fi
    
    # Portal 不存在，尝试创建
    log "未找到 Portal，尝试自动创建..."
    local create_body='{"name":"demo"}'
    if call_api "创建Portal" "POST" "${ADMIN_HOST}" "/api/v1/portals" "$create_body" "$extra_args"; then
      log "Portal 创建请求已发送"
    else
      log "Portal 创建请求返回 HTTP ${API_HTTP_CODE}（可能已存在）"
    fi
    ...
    if [[ -n "$PORTAL_ID" ]]; then
      log "Portal 创建成功，Portal ID: ${PORTAL_ID}"
      return 0
    fi
    
    err "确保 Portal 存在失败 (第 ${attempt} 次)"
    if (( attempt < MAX_RETRIES )); then
      log "等待 5 秒后重试..."
      sleep 5
    fi
...
# 步骤 3: 注册门户账号（幂等）
########################################
step_3_register_developer() {
  local body="{\"username\":\"${FRONT_USERNAME}\",\"password\":\"${FRONT_PASSWORD}\"}"
  
  local attempt=1
  while (( attempt <= MAX_RETRIES )); do
    log "执行步骤 3: 注册门户账号 (第 ${attempt}/${MAX_RETRIES} 次)..."
...
########################################
# 步骤 4: 获取 Developer ID
########################################
step_4_get_developer_info() {
  local extra_args="-H 'Authorization: Bearer ${ADMIN_TOKEN}'"
  
  # 查询该 Portal 下的开发者列表
  local attempt=1
  while (( attempt <= MAX_RETRIES )); do
    log "执行步骤 4: 获取 Developer ID (第 ${attempt}/${MAX_RETRIES} 次)..."
...
step_5_approve_developer() {
  ...
  local attempt=1
  while (( attempt <= MAX_RETRIES )); do
    log "执行步骤 5: 审批开发者 (第 ${attempt}/${MAX_RETRIES} 次)..."
...
# 步骤 6: 前台登录
########################################
step_6_frontend_login() {
  local body="{\"username\":\"${FRONT_USERNAME}\",\"password\":\"${FRONT_PASSWORD}\"}"
  
  local attempt=1
  while (( attempt <= MAX_RETRIES )); do
    log "执行步骤 6: 前台登录 (第 ${attempt}/${MAX_RETRIES} 次)..."
...
########################################
# 步骤 7: 获取 Consumer ID 和所有可订阅的产品列表
########################################
step_7_get_subscription_info() {
  # 如果步骤 6 已经获取到 Consumer ID，跳过此步骤
  if [[ -n "$CONSUMER_ID" ]]; then
    ...
    local attempt=1
    while (( attempt <= MAX_RETRIES )); do
      log "执行步骤 7.1: 获取 Consumer ID (第 ${attempt}/${MAX_RETRIES} 次)..."
  ...
  local attempt=1
  while (( attempt <= MAX_RETRIES )); do
    log "执行步骤 7.2: 获取可订阅产品列表 (第 ${attempt}/${MAX_RETRIES} 次)..."
...
main() {
  ...
  # 步骤 1: 管理员登录（获取 token，后续步骤依赖）
  if ! step_1_admin_login; then
    err "步骤 1 失败，终止执行"
    exit 1
  fi
  
  # 步骤 2: 确保 Portal 存在（注册开发者前必须有 Portal）
  if ! step_2_ensure_portal; then
    err "步骤 2 失败，终止执行"
    exit 1
  fi
  
  # 步骤 3: 注册门户账号
  if ! step_3_register_developer; then
    err "步骤 3 失败，终止执行"
    exit 1
  fi
  
  # 步骤 4: 获取 Developer ID
  if ! step_4_get_developer_info; then
    err "步骤 4 失败，终止执行"
    exit 1
  fi
  
  # 步骤 5: 审批开发者
  if ! step_5_approve_developer; then
    err "步骤 5 失败，终止执行"
    exit 1
  fi
  
  # 步骤 6: 前台登录
  if ! step_6_frontend_login; then
    err "步骤 6 失败，终止执行"
    exit 1
  fi
  
  # 步骤 7: 获取 Consumer ID 和 Product ID
  if ! step_7_get_subscription_info; then
    err "步骤 7 失败，终止执行"
    exit 1
  fi
  
  # 步骤 8: 订阅所有产品
  if ! step_8_subscribe_products; then
    err "步骤 8 失败，终止执行"
    exit 1
  fi
